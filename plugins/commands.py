...
async def start(client, message):
    ...
    if data.split("-", 1)[0] == "BATCH":
        ...
        for msg in msgs:
            ...
            if f_caption is None:
                f_caption = f"{title}"
            try:
                jhatu=await client.send_cached_media(
                ...
                randi=await client.send_cached_media(
                    chat_id=message.from_user.id,
                    file_id=msg.get("file_id"),
                    caption=f_caption,
                    
                    reply_markup=InlineKeyboardMarkup( [ [InlineKeyboardButton("🍿 Join Channel 🍿", url="https://t.me/movies_x_store")] ] ),
    ...
    huhh=await client.send_cached_media(
        chat_id=message.from_user.id,
        file_id=file_id,
        caption=f_caption,
